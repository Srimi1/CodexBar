...
extension CostUsageScanner {
        ...
        checkCancellation: CancellationCheck? = nil) throws -> CostUsageDailyReport
    {
        ...
        var cache = CostUsageMuseCacheIO.load(cacheRoot: options.cacheRoot, calendar: range.calendar)
        ...
        if shouldRefresh {
            ...
            try checkCancellation?()
            CostUsageMuseCacheIO.save(cache: cache, cacheRoot: options.cacheRoot, calendar: range.calendar)
        }
        ...
        checkCancellation: CancellationCheck?) throws -> MuseParseResult
    {
        ...
        return MuseParseResult(rows: collector.rows, parsedBytes: parsedBytes)
    }
        ...
        checkCancellation: CancellationCheck?) throws -> MuseParseResult
    {
        ...
        func collect(_ obj: [String: Any]) {
            if let parsed = self.parseMuseObject(obj: obj, range: range, fallbackSessionId: fallbackId) {
                collector.append(parsed.row, source: parsed.source)
            }
        }
        ...
        return MuseParseResult(rows: collector.rows, parsedBytes: Int64(data.count))
    }
...
enum CostUsageMuseCacheIO {
    ...
    static func cacheFileURL(cacheRoot: URL? = nil) -> URL {
        ...
        return root
            ...
            .appendingPathComponent("muse-v1.json", isDirectory: false)
    }

    static func load(cacheRoot: URL? = nil, calendar: Calendar? = nil) -> CostUsageCache {
    ...
    static func save(cache: CostUsageCache, cacheRoot: URL? = nil, calendar: Calendar = .current) {
        let url = self.cacheFileURL(cacheRoot: cacheRoot)
        var cache = cache
