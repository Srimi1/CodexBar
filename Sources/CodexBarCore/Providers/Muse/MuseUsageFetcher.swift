...
public struct MuseUsageFetcher: Sendable {
    ...
    private static func tokensFromFile(fileURL: URL, since: Date, todayStart: Date) -> (today: Int, weekly: Int) {
        ...
        var today = 0
        var weekly = 0
        // Muse CLI logs each model call as both `model_completed` and `goal_usage_attribution`; count one kind only.
        var completed = (today: 0, weekly: 0)
        var attributed = (today: 0, weekly: 0)
        var generic = (today: 0, weekly: 0)
        ...
        func processObject(_ json: [String: Any]) {
            ...
            var kind = 0 // 0 generic, 1 model_completed, 2 attribution
            let totalTokens: Int = {
            ...
            switch kind {
            case 1:
                completed.weekly += totalTokens
                if isToday { completed.today += totalTokens }
            case 2:
                attributed.weekly += totalTokens
                if isToday { attributed.today += totalTokens }
            default:
                generic.weekly += totalTokens
                if isToday { generic.today += totalTokens }
            }
        }

        func finish() -> (today: Int, weekly: Int) {
            let primary = (completed.today > 0 || completed.weekly > 0) ? completed : attributed
            today = primary.today + generic.today
            weekly = primary.weekly + generic.weekly
            return (today, weekly)
        }
